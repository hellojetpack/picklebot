module.exports = (controller) => {
  ...
  controller.hears(['avg game time', 'average game time'], 'direct_mention, mention', (bot, message) => {
    ...
    controller.storage.teams.get(message.team, (err, teamData) => {
      if (err) {
        bot.say('I had trouble receiving that from my database.');
      }
      bot.startConversation(message, (converr, convo) => {
        ...
        if (!teamData || !teamData.gameData || !teamData.gameData.avgTime) {
          ...
          convo.say(`Here is your team's avg game time: ${teamData.gameData.avgTime}`);
